...
static void softirq_handler(struct softirq_action *action)
{
	...
	printk(KERN_INFO"softirq handler called\n");
	retrun 0;
...
static int __init init_module (void)
{
    ...
    open_softirq( MY_SOFTIRQ, softirq_handler );//Registring softirq handler with softirq
...
MODULE_DESCRIPTION ("sample interrupt handler on softirqs");
MODULE_LICENSE ("GPL");
