...
int thread_function3(void *pv)
{
    while(!kthread_should_stop()) {
        read_lock(&my_rwlock);
        pr_info("In Thread Function - 3 : Read value %lu\n", test_global_variable);
        read_unlock(&my_rwlock);
        msleep(100);
    }
    return 0;
...
static int __init test_driver_init(void)
{
        ...
        /*Creating device*/
        if((device_create(dev_class,NULL,dev,NULL,"my_rwlock_device")) == NULL){
        ...
        /* Creating Thread 3 */
        my_thread3 = kthread_run(thread_function3,NULL,"my Thread3");
        if(my_thread3) {
